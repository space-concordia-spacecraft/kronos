...
int main(int argc, char * argv[]) {
    ...
    for (int i = 0; i <= CSP_LOCK; ++i) {
        csp_debug_set_level(static_cast<csp_debug_level_t>(i), true);
    }
    ...
    int res = csp_thread_create(thread_func, "thread", configMINIMAL_STACK_SIZE, NULL, 0, &thread);
...
//    csp_assert(csp_get_s_isr() >= (sec2 + 1));
    ...
    // malloc
    uint32_t * ptr = static_cast<uint32_t*>(csp_malloc(sizeof(*ptr)));
    csp_assert(ptr != NULL);
    ptr[0] = 10;
    csp_free(ptr);
    ptr = static_cast<uint32_t*>(csp_calloc(1, sizeof(*ptr)));
    csp_assert(ptr != NULL);
    csp_assert(*ptr == 0);
    ptr[0] = 20;
    csp_free(ptr);
